...
bool ParseIPLiteralToBytes(const absl::string_view& ip_literal,
  ...
  if (ip_literal.find(':') != absl::string_view::npos) {
    // GURL expects IPv6 hostnames to be surrounded with brackets.
    std::string host_brackets = "[" + std::string(ip_literal.data(), ip_literal.size()) + "]";
...
IPAddress ConvertIPv4MappedIPv6ToIPv4(const IPAddress& address) {
  ...
  bytes.insert(bytes.end(),
               address.bytes().begin() + internal::size(kIPv4MappedPrefix),
               address.bytes().end());
  return IPAddress(bytes.data(), bytes.size());
}
...
bool ParseCIDRBlock(absl::string_view cidr_literal,
  ...
  std::vector<absl::string_view> parts = absl::StrSplit(cidr_literal, "/");
  if (parts.size() != 2)
    return false;
