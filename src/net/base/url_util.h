...
// NOTE: QueryIterator stores reference to |url| and creates absl::string_view
// instances which refer to the data inside |url| query. Therefore |url| must
// outlive QueryIterator and all absl::string_view objects returned from GetKey
...
bool ParseHostAndPort(absl::string_view input,
                                 std::string* host,
                                 int* port);
...
void GetIdentityFromURL(const GURL& url, std::string* username,
                        std::string* password);
