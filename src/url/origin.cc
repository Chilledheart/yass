...
Origin Origin::Create(const GURL& url) {
  ...
  if (url.SchemeIsFileSystem()) {
    ...
    DCHECK(!tuple.IsValid() || url.IsStandard() ||
           Contains(GetLocalSchemes(), url.scheme_piece()) ||
...
absl::optional<Origin> Origin::UnsafelyCreateOpaqueOriginWithoutNormalization(
    absl::string_view precursor_scheme,
    absl::string_view precursor_host,
...
absl::optional<UnguessableToken> Origin::GetNonceForSerialization()
    const {
...
bool Origin::CanBeDerivedFrom(const GURL& url) const {
  ...
  if (Contains(url::GetNoAccessSchemes(), url.scheme()) &&
...
bool Origin::DomainIs(absl::string_view canonical_domain) const {
...
std::string Origin::GetDebugString(bool include_nonce) const {
  ...
  if (!opaque()) {
    ...
    if (scheme() == kFileScheme)
      out += " [internally: " + tuple_.Serialize() + "]";
    return out;
  }
  ...
  // understand.
  std::string out = Serialize() + " [internally:";
  if (include_nonce) {
    out += " (";
  ...
  if (!tuple_.IsValid())
    ...
    out += " derived from " + tuple_.Serialize() + "]";
  return out;
}
...
absl::optional<Origin> Origin::Deserialize(const std::string& value) {
  ...
  if (nonce_high != 0 && nonce_low != 0) {
    ...
    nonce = Origin::Nonce(
        UnguessableToken::Deserialize(nonce_high, nonce_low));
  }
  Origin origin;
...
Origin::Nonce::Nonce(const UnguessableToken& token) : token_(token) {
  CHECK(!token_.is_empty());
}

const UnguessableToken& Origin::Nonce::token() const {
  ...
  if (token_.is_empty())
    token_ = UnguessableToken::Create();
  return token_;
}

const UnguessableToken& Origin::Nonce::raw_token() const {
  return token_;
}
...
Origin::Nonce::Nonce(Origin::Nonce&& other) noexcept : token_(other.token_) {
  other.token_ = UnguessableToken();  // Reset |other|.
}
...
ScopedOriginCrashKey::ScopedOriginCrashKey(
    debug::CrashKeyString* crash_key,
