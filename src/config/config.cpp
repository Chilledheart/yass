...
ABSL_FLAG(std::string, server_host, DEFAULT_SERVER,
          "IP address which remote server listens to");
ABSL_FLAG(int32_t, server_port, DEFAULT_SERVER_PORT,
          "Port number which remote server listens to");
ABSL_FLAG(std::string, password, DEFAULT_PASS, "Password pharsal");
ABSL_FLAG(std::string, method, DEFAULT_CIPHER, "Method of encrypt");
ABSL_FLAG(std::string, local_host, DEFAULT_LOCAL,
          "IP address which local server listens to");
ABSL_FLAG(int32_t, local_port, DEFAULT_LOCAL_PORT,
          "Port number which local server listens to");
ABSL_FLAG(bool,reuse_port, DEFAULT_REUSE_PORT, "Reuse the listening port");
ABSL_FLAG(std::string, congestion_algorithm, DEFAULT_CONGESTION_ALGORITHM,
          "TCP Congestion Algorithm");
ABSL_FLAG(bool, tcp_fastopen, DEFAULT_TCP_FASTOPEN, "TCP fastopen");
ABSL_FLAG(bool, tcp_fastopen_connect, DEFAULT_TCP_FASTOPEN,
          "TCP fastopen connect");
ABSL_FLAG(bool, auto_start, DEFAULT_AUTO_START, "Auto Start");

ABSL_FLAG(int32_t, timeout, DEFAULT_CONNECT_TIMEOUT,
          "Connect timeout (Linux only)");
ABSL_FLAG(int32_t,
          tcp_user_timeout,
          DEFAULT_TCP_USER_TIMEOUT,
          "TCP user timeout (Linux only)");
ABSL_FLAG(int32_t, so_linger_timeout, DEFAULT_SO_LINGER_TIMEOUT,
          "SO Linger timeout");

ABSL_FLAG(int32_t, so_snd_buffer, DEFAULT_SO_SND_BUFFER, "Socket Send Buffer");
ABSL_FLAG(int32_t, so_rcv_buffer, DEFAULT_SO_RCV_BUFFER,
...
bool ReadConfig() {
  ...
  if (cipher_method_in_use == CRYPTO_INVALID) {
    LOG(WARNING) << "bad method: " << absl::GetFlag(FLAGS_method);
    return false;
  }
  ...
  VLOG(1) << "loaded option method: " << absl::GetFlag(FLAGS_method);
  ...
  absl::SetFlag(&FLAGS_timeout,
                std::max(MAX_CONNECT_TIMEOUT, absl::GetFlag(FLAGS_timeout)));
  ...
  VLOG(1) << "loaded option timeout: " << absl::GetFlag(FLAGS_timeout);
  ...
  VLOG(1) << "initializing ciphers... " << absl::GetFlag(FLAGS_method);
...
bool SaveConfig() {
  absl::SetFlag(&FLAGS_method, to_cipher_method_str(cipher_method_in_use));
  ...
  VLOG(1) << "saved option method: " << absl::GetFlag(FLAGS_method);
  ...
  VLOG(1) << "saved option auto_start: " << std::boolalpha
    << absl::GetFlag(FLAGS_auto_start);
  VLOG(1) << "saved option timeout: " << absl::GetFlag(FLAGS_timeout);
