...
void Utils::DisableGtkRTTI(Gtk::Window *window) {
  ...
  GTK_CONTAINER_GET_CLASS(cobj)->child_type =
      [](GtkContainer* container) -> GType {
        const auto base = static_cast<GtkContainerClass*>(
            g_type_class_peek_parent(G_OBJECT_GET_CLASS(G_OBJECT(container))));
        if (base && base->child_type)
          return base->child_type(container);
        return GType();
      };
  GTK_CONTAINER_GET_CLASS(cobj)->composite_name =
      [](GtkContainer* container, GtkWidget* p0) -> gchar* {
        const auto base = static_cast<GtkContainerClass*>(
            g_type_class_peek_parent(G_OBJECT_GET_CLASS(G_OBJECT(container))));
        if (base && base->composite_name)
          return base->composite_name(container, p0);
        return nullptr;
      };
  GTK_CONTAINER_GET_CLASS(cobj)->set_child_property =
      [](GtkContainer* container, GtkWidget* child, guint property_id,
         const GValue* value, GParamSpec* pspec) {
        const auto base = static_cast<GtkContainerClass*>(
            g_type_class_peek_parent(G_OBJECT_GET_CLASS(G_OBJECT(container))));
        if (base && base->set_child_property)
          base->set_child_property(container, child, property_id, value, pspec);
      };
  GTK_CONTAINER_GET_CLASS(cobj)->get_child_property =
      [](GtkContainer* container, GtkWidget* child, guint property_id,
         GValue* value, GParamSpec* pspec) {
        const auto base = static_cast<GtkContainerClass*>(
            g_type_class_peek_parent(G_OBJECT_GET_CLASS(G_OBJECT(container))));
        if (base && base->get_child_property)
          base->get_child_property(container, child, property_id, value, pspec);
      };
  GTK_CONTAINER_GET_CLASS(cobj)->get_path_for_child =
      [](GtkContainer* container, GtkWidget* child) -> GtkWidgetPath* {
    const auto base = static_cast<GtkContainerClass*>(
        g_type_class_peek_parent(G_OBJECT_GET_CLASS(G_OBJECT(container))));
    if (base && base->get_path_for_child)
      return base->get_path_for_child(container, child);
    return nullptr;
  };

  auto wobj = (Gtk::Container*)(window->gobj());

  GTK_WIDGET_GET_CLASS(wobj)->destroy = [](GtkWidget* widget) {
  ...
  GTK_WIDGET_GET_CLASS(wobj)->show_all = [](GtkWidget* widget) {
    const auto base = static_cast<GtkWidgetClass*>(
        g_type_class_peek_parent(G_OBJECT_GET_CLASS(G_OBJECT(widget))));
    if (base && base->show_all)
      base->show_all(widget);
  };
  ...
  GTK_WIDGET_GET_CLASS(wobj)->state_flags_changed =
      [](GtkWidget* widget, GtkStateFlags p0) {
        const auto base = static_cast<GtkWidgetClass*>(
            g_type_class_peek_parent(G_OBJECT_GET_CLASS(G_OBJECT(widget))));
        if (base && base->state_flags_changed)
          base->state_flags_changed(widget, p0);
      };
  GTK_WIDGET_GET_CLASS(wobj)->parent_set =
      [](GtkWidget* widget, GtkWidget* p0) {
        const auto base = static_cast<GtkWidgetClass*>(
            g_type_class_peek_parent(G_OBJECT_GET_CLASS(G_OBJECT(widget))));
        if (base && base->parent_set)
          base->parent_set(widget, p0);
      };
  GTK_WIDGET_GET_CLASS(wobj)->hierarchy_changed =
      [](GtkWidget* widget, GtkWidget* p0) {
        const auto base = static_cast<GtkWidgetClass*>(
            g_type_class_peek_parent(G_OBJECT_GET_CLASS(G_OBJECT(widget))));
        if (base && base->hierarchy_changed)
          base->hierarchy_changed(widget, p0);
      };
  GTK_WIDGET_GET_CLASS(wobj)->style_set = [](GtkWidget* widget, GtkStyle* p0) {
    const auto base = static_cast<GtkWidgetClass*>(
        g_type_class_peek_parent(G_OBJECT_GET_CLASS(G_OBJECT(widget))));
    if (base && base->style_set)
      base->style_set(widget, p0);
  };
  GTK_WIDGET_GET_CLASS(wobj)->direction_changed = [](GtkWidget* widget,
                                                     GtkTextDirection p0) {
    const auto base = static_cast<GtkWidgetClass*>(
        g_type_class_peek_parent(G_OBJECT_GET_CLASS(G_OBJECT(widget))));
    if (base && base->direction_changed)
      base->direction_changed(widget, p0);
  };
  ...
  /* Mnemonics */
  GTK_WIDGET_GET_CLASS(wobj)->mnemonic_activate =
      [](GtkWidget* widget, gboolean p0) -> gboolean {
        const auto base = static_cast<GtkWidgetClass*>(
            g_type_class_peek_parent(G_OBJECT_GET_CLASS(G_OBJECT(widget))));
        if (base && base->mnemonic_activate)
          return base->mnemonic_activate(widget, p0);
        return gboolean();
      };

  /* explicit focus */
  GTK_WIDGET_GET_CLASS(wobj)->grab_focus =
      [](GtkWidget* widget) {
        const auto base = static_cast<GtkWidgetClass*>(
            g_type_class_peek_parent(G_OBJECT_GET_CLASS(G_OBJECT(widget))));
        if (base && base->grab_focus)
          base->grab_focus(widget);
      };
  ...
  /* keyboard navigation */
  GTK_WIDGET_GET_CLASS(wobj)->move_focus =
      [](GtkWidget* widget, GtkDirectionType p0) {
    const auto base = static_cast<GtkWidgetClass*>(
        g_type_class_peek_parent(G_OBJECT_GET_CLASS(G_OBJECT(widget))));
    if (base && base->move_focus)
      base->move_focus(widget, p0);
  };
  GTK_WIDGET_GET_CLASS(wobj)->keynav_failed =
      [](GtkWidget* widget, GtkDirectionType p0) -> gboolean {
    const auto base = static_cast<GtkWidgetClass*>(
        g_type_class_peek_parent(G_OBJECT_GET_CLASS(G_OBJECT(widget))));
    if (base && base->keynav_failed)
      return base->keynav_failed(widget, p0);
    return gboolean();
  };
  ...
  GTK_WIDGET_GET_CLASS(wobj)->scroll_event =
      [](GtkWidget* widget, GdkEventScroll* p0) -> gboolean {
    const auto base = static_cast<GtkWidgetClass*>(
        g_type_class_peek_parent(G_OBJECT_GET_CLASS(G_OBJECT(widget))));
    if (base && base->scroll_event)
      return base->scroll_event(widget, p0);
    return gboolean();
  };
  ...
  GTK_WIDGET_GET_CLASS(wobj)->unmap_event =
      [](GtkWidget* widget, GdkEventAny *p0) -> gboolean {
        const auto base = static_cast<GtkWidgetClass*>(
            g_type_class_peek_parent(G_OBJECT_GET_CLASS(G_OBJECT(widget))));
        if (base && base->unmap_event)
          return base->unmap_event(widget, p0);
        return gboolean();
      };
  GTK_WIDGET_GET_CLASS(wobj)->property_notify_event =
      [](GtkWidget* widget, GdkEventProperty *p0) -> gboolean {
        const auto base = static_cast<GtkWidgetClass*>(
            g_type_class_peek_parent(G_OBJECT_GET_CLASS(G_OBJECT(widget))));
        if (base && base->property_notify_event)
          return base->property_notify_event(widget, p0);
        return gboolean();
      };
  GTK_WIDGET_GET_CLASS(wobj)->selection_clear_event =
      [](GtkWidget* widget, GdkEventSelection *p0) -> gboolean {
        const auto base = static_cast<GtkWidgetClass*>(
            g_type_class_peek_parent(G_OBJECT_GET_CLASS(G_OBJECT(widget))));
        if (base && base->selection_clear_event)
          return base->selection_clear_event(widget, p0);
        return gboolean();
      };
  GTK_WIDGET_GET_CLASS(wobj)->selection_request_event =
      [](GtkWidget* widget, GdkEventSelection *p0) -> gboolean {
        const auto base = static_cast<GtkWidgetClass*>(
            g_type_class_peek_parent(G_OBJECT_GET_CLASS(G_OBJECT(widget))));
        if (base && base->selection_request_event)
          return base->selection_request_event(widget, p0);
        return gboolean();
      };
  GTK_WIDGET_GET_CLASS(wobj)->selection_notify_event =
      [](GtkWidget* widget, GdkEventSelection *p0) -> gboolean {
        const auto base = static_cast<GtkWidgetClass*>(
            g_type_class_peek_parent(G_OBJECT_GET_CLASS(G_OBJECT(widget))));
        if (base && base->selection_notify_event)
          return base->selection_notify_event(widget, p0);
        return gboolean();
      };
  GTK_WIDGET_GET_CLASS(wobj)->proximity_in_event =
      [](GtkWidget* widget, GdkEventProximity *p0) -> gboolean {
        const auto base = static_cast<GtkWidgetClass*>(
            g_type_class_peek_parent(G_OBJECT_GET_CLASS(G_OBJECT(widget))));
        if (base && base->proximity_in_event)
          return base->proximity_in_event(widget, p0);
        return gboolean();
      };
  GTK_WIDGET_GET_CLASS(wobj)->proximity_out_event =
      [](GtkWidget* widget, GdkEventProximity *p0) -> gboolean {
        const auto base = static_cast<GtkWidgetClass*>(
            g_type_class_peek_parent(G_OBJECT_GET_CLASS(G_OBJECT(widget))));
        if (base && base->proximity_out_event)
          return base->proximity_out_event(widget, p0);
        return gboolean();
      };
  ...
  GTK_WIDGET_GET_CLASS(wobj)->damage_event =
      [](GtkWidget* widget, GdkEventExpose* p0) -> gboolean {
    const auto base = static_cast<GtkWidgetClass*>(
        g_type_class_peek_parent(G_OBJECT_GET_CLASS(G_OBJECT(widget))));
    if (base && base->damage_event)
      return base->damage_event(widget, p0);
    return gboolean();
  };
  GTK_WIDGET_GET_CLASS(wobj)->grab_broken_event =
      [](GtkWidget* widget, GdkEventGrabBroken* p0) -> gboolean {
    const auto base = static_cast<GtkWidgetClass*>(
        g_type_class_peek_parent(G_OBJECT_GET_CLASS(G_OBJECT(widget))));
    if (base && base->grab_broken_event)
      return base->grab_broken_event(widget, p0);
    return gboolean();
  };
