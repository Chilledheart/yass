...
#ifndef CORE_STRING_UTIL_INTERNAL_H
#define CORE_STRING_UTIL_INTERNAL_H
...
template <typename T, typename CharT = typename T::value_type>
T TrimStringViewT(T input, T trim_chars, TrimPositions positions) {
  size_t begin =
      (positions & TRIM_LEADING) ? input.find_first_not_of(trim_chars) : 0;
...
#endif  // CORE_STRING_UTIL_INTERNAL_H
