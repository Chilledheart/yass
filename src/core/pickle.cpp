...
bool PickleIterator::ReadLong(long* result) {
  ...
  *result = checked_cast<long>(result_int64);
  return true;
}
...
bool PickleIterator::ReadStringView(absl::string_view* result) {
  int len;
  if (!ReadInt(&len))
    return false;
  const char* read_from = GetReadPointerAndAdvance(len);
  ...
  *result = absl::string_view(read_from, len);
  return true;
}
...
bool PickleIterator::ReadData(span<const uint8_t>* data) {
  const char* ptr;
  int length;
  ...
  *data = as_bytes(make_span(ptr, length));
  return true;
}
...
Pickle::Pickle()
    : header_(nullptr),
      ...
      write_offset_(0) {
  static_assert(bits::IsPowerOfTwo(Pickle::kPayloadUnit),
...
void Pickle::WriteString(const absl::string_view& value) {
  WriteInt(static_cast<int>(value.size()));
  WriteBytes(value.data(), static_cast<int>(value.size()));
}

void Pickle::WriteString16(const std::u16string& value) {
  WriteInt(static_cast<int>(value.size()));
...
bool Pickle::ReadAttachment(PickleIterator* iter,
