...
#ifndef CORE_RANGES_ALGORITHM_H_
#define CORE_RANGES_ALGORITHM_H_
...
#include "core/functional/identity.hpp"
#include "core/functional/invoke.hpp"
#include "core/ranges/functional.hpp"
#include "core/ranges/ranges.hpp"
...
constexpr bool equal(ForwardIterator1 first1,
  ...
  if (is_constant_evaluated()) {
    for (; first1 != last1 && first2 != last2; ++first1, ++first2) {
      if (!invoke(pred, invoke(proj1, *first1), invoke(proj2, *first2))) {
        return false;
      }
    }
...
constexpr auto transform(ForwardIterator1 first1,
  ...
  return std::transform(
      ...
      [&binary_op, &proj1, &proj2](auto&& lhs, auto&& rhs) {
        return invoke(
            binary_op, invoke(proj1, std::forward<decltype(lhs)>(lhs)),
...
constexpr auto replace(ForwardIterator first,
  ...
  std::replace_if(
      ...
      [&proj, &old_value](auto&& lhs) {
        return invoke(proj, std::forward<decltype(lhs)>(lhs)) ==
               old_value;
      },
      new_value);
  return last;
...
constexpr auto is_sorted_until(ForwardIterator first,
  ...
  for (ForwardIterator next = first; ++next != last; ++first) {
    if (invoke(comp, invoke(proj, *next), invoke(proj, *first))) {
      return next;
    }
  }
...
constexpr const T& min(const T& a, const T& b, Comp comp = {}, Proj proj = {}) {
  return invoke(comp, invoke(proj, b), invoke(proj, a)) ? b
                                                                          : a;
}
...
constexpr const T& max(const T& a, const T& b, Comp comp = {}, Proj proj = {}) {
  return invoke(comp, invoke(proj, a), invoke(proj, b)) ? b
                                                                          : a;
}
...
#endif  // CORE_RANGES_ALGORITHM_H_
