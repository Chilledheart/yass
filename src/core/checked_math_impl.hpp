...
#ifndef CORE_CHECKED_MATH_IMPL_H_
#define CORE_CHECKED_MATH_IMPL_H_
...
#include "core/safe_math_shared_impl.hpp"
...
struct CheckedAddOp<T,
  ...
  static constexpr bool Do(T x, U y, V* result) {
    ...
    if (NUMERICS_UNLIKELY(!IsValueInRangeForNumericType<Promotion>(x) ||
...
struct CheckedDivOp<T,
  ...
  template <typename V>
  static constexpr bool Do(T x, U y, V* result) {
    if (NUMERICS_UNLIKELY(!y))
      return false;

    // The overflow check can be compiled away if we don't have the exact
    // combination of types needed to trigger this case.
    using Promotion = typename BigEnoughPromotion<T, U>::type;
    if (NUMERICS_UNLIKELY(
...
struct CheckedRshOp<T,
  ...
  static constexpr bool Do(T x, U shift, V* result) {
    ...
    if (NUMERICS_UNLIKELY(as_unsigned(shift) >=
...
#endif  // CORE_CHECKED_MATH_IMPL_H_
