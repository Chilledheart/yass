...
size_t FastHashImpl(const uint8_t* data, size_t length) {
...
size_t FastHash(const uint8_t *data, size_t length) {
  return Scramble(FastHashImpl(data, length));
}
...
uint32_t Hash(const std::u16string& str) {
  return PersistentHash(str.data(), str.size() * sizeof(char16_t));
}
...
uint32_t PersistentHash(const void* data, size_t length) {
  ...
  return ::FastHashImpl(reinterpret_cast<const uint8_t*>(data),
