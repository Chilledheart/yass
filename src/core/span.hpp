...
#ifndef CORE_CONTAINERS_SPAN_H_
#define CORE_CONTAINERS_SPAN_H_
...
#include "core/checked_iterators.hpp"
#include "core/contiguous_iterator.hpp"
#include "core/cxx17_backports.hpp"
#include "core/cxx20_to_address.hpp"
...
using ContainerHasIntegralSize =
    std::is_integral<decltype(size(std::declval<Container>()))>;
...
class span : public internal::ExtentStorage<Extent> {
 ...
 public:
  ...
  // Conversion from a container that has compatible data() and integral size().
  template <
      typename Container,
      typename =
  ...
  constexpr span(Container& container) noexcept
      : span(data(container), size(container)) {}
...
//
// Usage: auto static_span = make_span<N>(...);
template <size_t N,
...
// EXTENT returns the size of any type that can be converted to a |span|
...
// context. Note:
//   * |size| should be preferred for plain arrays.
//   * In run-time contexts, functions such as |std::array::size| should be
//     preferred.
#define EXTENT(x)                                  \
  ::internal::must_not_be_dynamic_extent<decltype( \
      ::make_span(x))::extent>()

#endif  // CORE_CONTAINERS_SPAN_H_
