...
#ifndef CORE_STRINGS_STRING_NUMBER_CONVERSIONS_INTERNAL_H_
#define CORE_STRINGS_STRING_NUMBER_CONVERSIONS_INTERNAL_H_
...
auto StringToNumber(const CharT* begin, const CharT *end) {
...
bool StringToIntImpl(T input, VALUE& output) {
  auto result = StringToNumber<VALUE, 10, CharT>(&*input.begin(), &*input.end());
  output = result.value;
  return result.valid;
}
...
bool HexStringToIntImpl(T input, VALUE& output) {
  auto result = StringToNumber<VALUE, 16, CharT>(&*input.begin(), &*input.end());
  output = result.value;
  return result.valid;
}
...
template <>
std::wstring DoubleToStringT(double value) {
...
#endif  // CORE_STRINGS_STRING_NUMBER_CONVERSIONS_INTERNAL_H_
