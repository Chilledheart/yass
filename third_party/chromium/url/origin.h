...
class Origin {
 public:
  ...
  const std::string& scheme() const {
    return !opaque() ? tuple_.scheme() : EmptyString();
  }
  const std::string& host() const {
    return !opaque() ? tuple_.host() : EmptyString();
  }
...
#ifdef OS_ANDROID
  android::ScopedJavaLocalRef<jobject> CreateJavaObject() const;
  static Origin FromJavaObject(
      const android::JavaRef<jobject>& java_origin);
#endif  // OS_ANDROID
 ...
 private:
  ...
  // Origin::Nonce is a wrapper around UnguessableToken that generates
  ...
  class Nonce {
   ...
   private:
    ...
    mutable UnguessableToken token_;
  };
  ...
  absl::optional<UnguessableToken> GetNonceForSerialization() const;
...
class ScopedOriginCrashKey
    : public debug::ScopedCrashKeyString {
 public:
  ScopedOriginCrashKey(debug::CrashKeyString* crash_key,
