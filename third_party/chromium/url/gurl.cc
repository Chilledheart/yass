...
GURL::GURL(absl::string_view url_string) {
...
GURL GURL::Resolve(absl::string_view relative) const {
...
bool GURL::SchemeIs(absl::string_view lower_ascii_scheme) const {
...
bool GURL::SchemeIsCryptographic(absl::string_view lower_ascii_scheme) {
  DCHECK(IsStringASCII(lower_ascii_scheme));
  DCHECK(ToLowerASCII(lower_ascii_scheme) == lower_ascii_scheme);
...
absl::string_view GURL::PathForRequestPiece() const {
  ...
  return absl::string_view(&spec_[parsed_.path.begin], path_len);
}
...
absl::string_view GURL::HostNoBracketsPiece() const {
...
bool GURL::DomainIs(absl::string_view canonical_domain) const {
  if (!is_valid_)
    return false;
...
bool GURL::EqualsIgnoringRef(const GURL& other) const {
  ...
  return absl::string_view(spec_).substr(0, ref_position) ==
         absl::string_view(other.spec_).substr(0, ref_position_other);
}
...
bool GURL::IsAboutUrl(absl::string_view allowed_path) const {
...
// static
bool GURL::IsAboutPath(absl::string_view actual_path,
                       absl::string_view allowed_path) {
  if (!StartsWith(actual_path, allowed_path))
    return false;
