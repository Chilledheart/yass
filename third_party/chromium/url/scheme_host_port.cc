...
bool IsCanonicalHost(const absl::string_view& host) {
  std::string canon_host;

  // Try to canonicalize the host (copy/pasted from net/base. :( ).
  const Component raw_host_component(0, checked_cast<int>(host.length()));
...
// differences).
bool IsValidInput(const absl::string_view& scheme,
  ...
  if (Contains(GetNoAccessSchemes(), scheme))
    return false;

  SchemeType scheme_type = SCHEME_WITH_HOST_PORT_AND_USER_INFORMATION;
  bool is_standard = GetStandardSchemeType(
      scheme.data(),
      Component(0, checked_cast<int>(scheme.length())),
      &scheme_type);
  if (!is_standard) {
...
SchemeHostPort::SchemeHostPort(absl::string_view scheme,
...
SchemeHostPort::SchemeHostPort(const GURL& url) {
  ...
  absl::string_view scheme = url.scheme_piece();
  absl::string_view host = url.host_piece();
...
std::string SchemeHostPort::SerializeInternal(url::Parsed* parsed) const {
  ...
  if (port_ != default_port) {
    result.push_back(':');
    std::string port(NumberToString(port_));
