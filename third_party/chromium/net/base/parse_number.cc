...
bool StringToNumber(const absl::string_view& input, int32_t* output) {
  // This assumes ints are 32-bits (will fail compile if that ever changes).
  return StringToInt(input, output);
}

bool StringToNumber(const absl::string_view& input, uint32_t* output) {
  // This assumes ints are 32-bits (will fail compile if that ever changes).
  return StringToUint(input, output);
}

bool StringToNumber(const absl::string_view& input, int64_t* output) {
  return StringToInt64(input, output);
}

bool StringToNumber(const absl::string_view& input, uint64_t* output) {
...
template <typename T>
bool ParseIntHelper(const absl::string_view& input,
  ...
  absl::string_view numeric_portion =
...
bool ParseInt32(const absl::string_view& input,
                ParseIntFormat format,
                int32_t* output,
                ParseIntError* optional_error) {
...
bool ParseInt64(const absl::string_view& input,
                ParseIntFormat format,
                int64_t* output,
                ParseIntError* optional_error) {
  return ParseIntHelper(input, format, output, optional_error);
...
bool ParseUint32(const absl::string_view& input,
                 uint32_t* output,
                 ParseIntError* optional_error) {
...
bool ParseUint64(const absl::string_view& input,
                 uint64_t* output,
                 ParseIntError* optional_error) {
  return ParseIntHelper(input, ParseIntFormat::NON_NEGATIVE, output,
                        optional_error);
