...
bool IsNormalizedLocalhostTLD(const std::string& host) {
  return EndsWith(host, ".localhost");
}
...
std::string UnescapeIdentityString(absl::string_view escaped_text) {
  std::string unescaped_text;
  if (UnescapeBinaryURLComponentSafe(
          escaped_text, false /* fail_on_path_separators */, &unescaped_text)) {
    return unescaped_text;
  }
  return std::string(escaped_text.data(), escaped_text.size());
}
...
absl::string_view QueryIterator::GetKey() const {
  DCHECK(!at_end_);
  if (key_.is_nonempty())
    return absl::string_view(&url_.spec()[key_.begin], key_.len);
  return absl::string_view();
}

absl::string_view QueryIterator::GetValue() const {
  DCHECK(!at_end_);
  if (value_.is_nonempty())
    return absl::string_view(&url_.spec()[value_.begin], value_.len);
  return absl::string_view();
}
...
bool ParseHostAndPort(absl::string_view input, std::string* host, int* port) {
  if (input.empty())
    return false;
...
std::string GetHostAndPort(const GURL& url) {
  ...
  return StringPrintf("%s:%d", url.host().c_str(),
...
std::string GetHostAndOptionalPort(
  ...
  if (default_port != scheme_host_port.port()) {
    return StringPrintf("%s:%i", scheme_host_port.host().c_str(),
                        scheme_host_port.port());
  }
  return scheme_host_port.host();
}

std::string TrimEndingDot(absl::string_view host) {
  absl::string_view host_trimmed = host;
...
std::string GetSuperdomain(absl::string_view domain) {
...
bool IsSubdomainOf(absl::string_view subdomain, absl::string_view superdomain) {
  ...
  if (!EndsWith(subdomain, superdomain))
    return false;
...
bool HostStringIsLocalhost(absl::string_view host) {
...
bool IsStandardSchemeWithNetworkHost(absl::string_view scheme) {
...
bool IsGoogleHost(absl::string_view host) {
...
bool IsLocalHostname(absl::string_view host) {
  std::string normalized_host = ToLowerASCII(host);
  // Remove any trailing '.'.
