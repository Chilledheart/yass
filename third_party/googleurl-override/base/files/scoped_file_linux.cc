...
NOINLINE void CrashOnFdOwnershipViolation() {
  // GURL_RAW_LOG(ERROR, "Crashing due to FD ownership violation:\n");
  assert(0 && "Crashing due to FD ownership violation");
  abort();
}
...
__attribute__((visibility("default"), noinline)) int close(int fd) {
  ...
  if (gurl_base::IsFDOwned(fd) && g_is_ownership_enforced)
    CrashOnFdOwnershipViolation();
  if (libc_close == nullptr) {
    // GURL_RAW_LOG(ERROR, "close symbol missing\n");
    assert(0 && "close symbol missing");
    abort();
  }
