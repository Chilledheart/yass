...
int RandInt(int min, int max) {
  ...
  int result =
      static_cast<int>(min + static_cast<int64_t>(gurl_base::RandGenerator(range)));
  GURL_DCHECK_GE(result, min);
  GURL_DCHECK_LE(result, max);
  return result;
}

double RandDouble() {
  return BitsToOpenEndedUnitInterval(gurl_base::RandUint64());
}

float RandFloat() {
  return BitsToOpenEndedUnitIntervalF(gurl_base::RandUint64());
}
...
InsecureRandomGenerator::InsecureRandomGenerator() {
  a_ = gurl_base::RandUint64();
  b_ = gurl_base::RandUint64();
}
...
MetricsSubSampler::ScopedDisableForTesting::ScopedDisableForTesting() {
  GURL_DCHECK(g_subsampling_enabled);
